...
struct EquilibriumManager
    ...
    Fe     :: Matrix{Float64}
    Fi     :: Matrix{Float64}
    Fe_prime :: Tuple{Matrix{Float64}, Matrix{Float64}}
    Fi_prime :: Tuple{Matrix{Float64}, Matrix{Float64}}
    ...
    function EquilibriumManager(coef, mesh_x, mesh_v, init_eq = true)
        ...
        if init_eq
            ...
            Fe_prime = Fe_prime_init(coef, mesh_x, mesh_v)
            Fi_prime = Fi_prime_init(coef, mesh_x, mesh_v)
        end

        new( mesh_x, mesh_v, coef, Fe, Fi, Fe_prime, Fi_prime)
    end
end
...
function compute_Fe(coef, mesh_x, mesh_v, exp_a_phi_x, scale_coef=false)
    ...
    return sqrt.(-a / pi) .* (s1 .* (exp_a_phi_x.^(-2))
        .* exp_a_v2 .+ (s2 ./ sqrt(2)) .* (exp_a_phi_x.^(-1))
        .* exp_a_v2_div_2)
end
...
function compute_Fi(coef, mesh_x, mesh_v, exp_a_phi_x, scale_coef=false)
    ...
    return sqrt.(-a ./ pi) .* (r1 .* (exp_a_phi_x.^2)
        .* exp_a_v2 .+ (r2 ./ sqrt(2)) .* exp_a_phi_x .* exp_a_v2_div_2)
end
...
function compute_Fe_prime(coef, mesh_x, mesh_v, exp_a_phi_x, dx_phi_x, scale_coef=false)
    ...
    f_eq_prime = sqrt.(-a / pi) .* (2 .* a .* s1 .* (
        exp_a_phi_x.^(-2)) .* exp_a_v2 .+ a .* (
        s2 ./ sqrt(2)) .* (exp_a_phi_x.^(-1)) .* exp_a_v2_div_2)

    dv_f_eq = similar(f_eq_prime)
    dx_f_eq = similar(f_eq_prime)

    @einsum dv_f_eq[i,j] = f_eq_prime[i,j] * v[j]

    @einsum dx_f_eq[i,j] = - f_eq_prime[i,j] * dx_phi_x[i]
